...
export function MembersTable({
    ...
    const filters: FilterDefinition[] = [
        {
            ...
            placeholder: t('members.admin.search_placeholder'),
            className: 'md:w-80',
        },
        ...(mode === 'all'
            ? [
                  {
                      ...
                      label: t('members.admin.filters.status'),
                      options: statuses,
                  },
              ]
            : []),
        {
            key: 'governorate_id',
            type: 'select',
            label: t('members.admin.filters.governorate'),
        ...
        },
        {
            key: 'joined',
            type: 'daterange',
            label: t('members.admin.filters.joined'),
        },
            ...
            ? [
                  {
                      key: 'referral_source',
                      type: 'select' as const,
                      label: t('members.admin.filters.referral_source'),
    ...
    const columns: DataTableColumn<MemberRow>[] = [
                ...
                <div className="min-w-0">
                    ...
                    {row.user_status === 'disabled' ? (
                        <p className="text-xs text-danger">
                            {t('members.admin.account_disabled')}
                        </p>
                    ) : null}
                </div>
            ),
        ...
        {
            key: 'member_number',
            header: t('members.admin.columns.member_number'),
            sortable: true,
            cell: (row) => <Code>{row.member_number}</Code>,
        },
        {
            key: 'mobile',
            header: t('members.admin.columns.contact'),
            cell: (row) =>
                row.mobile ? (
                    <PhoneNumber value={row.mobile} />
                ) : (
        ...
        {
            key: 'governorate',
            header: t('members.admin.columns.governorate'),
            hideOnMobile: true,
        },
        {
            key: 'status',
            header: t('members.admin.columns.status'),
            sortable: true,
            cell: (row) => <MembershipStatusBadge status={row.status} />,
        },
        {
            key: 'joined_at',
            header: t('members.admin.columns.joined_at'),
            sortable: true,
            cell: (row) => <DateTime value={row.joined_at} mode="date" />,
        },
        {
            key: 'referral_source',
            header: t('members.admin.columns.referral_source'),
            defaultHidden: true,
            hideOnMobile: true,
        },
    ];
    ...
    const confirmBulk = () => {
        ...
        const ids = members.data
            .filter(
                (row) => bulk.ids.includes(row.id) && row.status === 'pending',
            )
            .map((row) => row.id);
        setProcessing(true);
        router.post(
    ...
    const selectedPending = (ids: RowKey[]) =>
        members.data.filter(
        ...
        <div className="flex flex-col gap-4">
            {mode === 'all' ? (
                <StatusChips
                    counts={counts}
                    active={
                        (query.get('status') as MembershipStatus | undefined) ??
                        null
                    }
                />
            ) : null}
            <DataTable<MemberRow>
                ...
                bulkActions={(selected, clear) =>
                    canApprove ? (
                        <Button
                            type="button"
                            size="sm"
                            onClick={() => setBulk({ ids: selected, clear })}
                            disabled={selectedPending(selected) === 0}
                        >
                ...
                caption={
                    mode === 'pending'
                        ? t('members.admin.pending_title')
                        : t('members.admin.title')
                }
                emptyTitle={
                    mode === 'pending'
                        ? t('members.admin.pending_empty')
                        : t('members.admin.empty')
                }
                emptyDescription={
                    mode === 'pending'
                        ? t('members.admin.pending_empty_hint')
                        : t('members.admin.empty_hint')
                }
                mobileTitle={(row) => (
            ...
            <ConfirmDialog
                open={bulk !== null}
                onOpenChange={(open) =>
                    !open && !processing ? setBulk(null) : undefined
                }
                title={t('members.admin.confirm.bulk_title', {
                    count: bulk ? selectedPending(bulk.ids) : 0,
                })}
...
function StatusChips({
    ...
    counts: MemberListProps['counts'];
    active: MembershipStatus | null;
}) {
    const query = useQueryState();
    const items: { key: MembershipStatus | 'total'; label: string }[] = [
        { key: 'total', label: t('members.admin.counts.total') },
    ...
    return (
        <nav
            aria-label={t('members.admin.counts.label')}
        ...
        >
            {items.map((item) => {
                ...
                return (
                    ...
                    >
                        {item.label}
                        <span className="tabular rounded-full bg-muted px-1.5 text-xs">
