...
export function MemberStatusActions({
    ...
    const run = (reason?: string) => {
        ...
        setProcessing(true);
        router.post(
            pending.route(membership.id).url,
            reason ? { reason } : {},
            {
                preserveScroll: true,
    ...
    return (
        <>
            ...
            <ConfirmDialog
                open={pending !== null}
                onOpenChange={(open) =>
                    !open && !processing ? setPending(null) : undefined
                }
                title={
                    pending
                        ? t(`members.admin.confirm.${pending.key}_title`)
                        : ''
                }
                description={
                    pending
                        ? t(`members.admin.confirm.${pending.key}_text`)
                        : undefined
                }
                confirmLabel={
                    pending
                        ? t(`members.admin.actions.${pending.key}`)
                        : undefined
                }
