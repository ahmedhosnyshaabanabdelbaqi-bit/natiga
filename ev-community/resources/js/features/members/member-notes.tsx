...
export function MemberNotes({
    ...
    const togglePin = (note: MemberNote) => {
        router.patch(
            pin({ membership: membershipId, note: note.id }).url,
            {},
            {
                preserveScroll: true,
            ...
            {canWrite ? (
                <SectionCard title={t('members.admin.notes.add')}>
                    <form onSubmit={submit} className="grid gap-3" noValidate>
                        <FormField
                            label={t('members.admin.notes.title')}
                            hideLabel
                        ...
                        >
                            <Textarea
                                ...
                                maxLength={2000}
                                placeholder={t(
                                    'members.admin.notes.placeholder',
                                )}
                            />
                        </FormField>
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <div className="flex items-center gap-2">
                                ...
                                <Label htmlFor="note-pinned">
                                    {t('members.admin.notes.pin_on_create')}
                                </Label>
                            </div>
                            <Button
                                type="submit"
            ...
            <SectionCard title={t('members.admin.notes.title')} flush>
                {notes.length === 0 ? (
                    <div className="p-4">
                        <EmptyState
                            icon={StickyNote}
                            title={t('members.admin.notes.empty')}
                        />
                    </div>
                ) : (
                        ...
                        {notes.map((note) => (
                            <li
                                ...
                                className={cn(
                                    ...
                                    note.is_pinned && 'bg-warning-soft/30',
                                )}
                            >
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                        ...
                                        <span>
                                            {note.author ??
                                                t(
                                                    'members.admin.membership.system',
                                                )}
                                        </span>
                                ...
                                {canWrite ? (
                                    <Button
                                        ...
                                        aria-label={
                                            note.is_pinned
                                                ? t('members.admin.notes.unpin')
                                                : t('members.admin.notes.pin')
                                        }
                                        title={
                                            note.is_pinned
                                                ? t('members.admin.notes.unpin')
