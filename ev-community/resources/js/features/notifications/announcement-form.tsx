...
import { audienceParamsToStrings } from '@/features/notifications/audience';
import type {
    AudienceField,
    AudienceOption,
    Campaign,
...
function initialData(props: Props): AnnouncementFormData {
    ...
    return {
        ...
        audience_params: audienceParamsToStrings(c?.audience_params),
    };
}
...
export function AnnouncementForm(props: Props) {
    ...
    const audience =
        audiences.find((a) => a.key === form.data.audience_type) ?? null;
        ...
        async (type: string, params: Record<string, string>) => {
            ...
            try {
                const result = await requestJson<{
                    ...
                    'post',
                    estimate.url(),
                    { audience_type: type, audience_params: params },
                    controller.signal,
                );
                if (result.ok) {
                    ...
                    setEstimateError(
                        Object.values(result.errors)[0] ??
                            result.message ??
                            t('notifications.errors.request_failed'),
                    );
                }
            } catch {
    ...
    useEffect(() => {
        const current = audiences.find(
            (a) => a.key === form.data.audience_type,
        );
        if (!current) {
            return;
        }
        const complete = current.fields.every(
            (field) =>
                (form.data.audience_params[field.name] ?? '').trim() !== '',
        );
        if (!complete) {
            setEstimateLabel(null);
            setEstimateError(null);
        ...
        const timer = window.setTimeout(
            () => void runEstimate(current.key, form.data.audience_params),
    ...
    const loadPreview = async () => {
        ...
        const result = await requestJson<Record<'ar' | 'en', LocalePreview>>(
            'post',
            preview.url(),
            { title_ar, title_en, body_ar, body_en, url },
        );
        setPreviewing(false);
        if (result.ok) {
            ...
            setPreviewError(
                result.message ?? t('notifications.errors.request_failed'),
            );
        }
    };

    const setParam = (name: string, value: string) =>
        form.setData('audience_params', {
            ...form.data.audience_params,
            [name]: value,
        });
    ...
    const toggleChannel = (value: string, checked: boolean) => {
        const next = checked
            ? [...new Set([...form.data.channels, value])]
            : form.data.channels.filter((c) => c !== value);
        form.setData(
            'channels',
            next.includes('in_app') ? next : ['in_app', ...next],
        );
    };
            ...
            return (
                ...
                >
                    {(controlProps) => (
                        <Select
                            value={value}
                            onValueChange={(next) => setParam(field.name, next)}
                        >
                            <SelectTrigger {...controlProps} className="w-full">
                                <SelectValue
                                    placeholder={t(
                                        'notifications.admin.fields.select_placeholder',
                                    )}
                                />
                            </SelectTrigger>
                            <SelectContent>
    ...
    return (
        ...
        >
            {error('domain') ? (
                <InlineAlert tone="danger">{error('domain')}</InlineAlert>
            ) : null}

            <SectionCard title={t('notifications.admin.fields.content')}>
                <div className="grid gap-4 md:grid-cols-2">
                    <FormField
                        label={t('notifications.admin.fields.title_ar')}
                    ...
                    <FormField
                        label={t('notifications.admin.fields.title_en')}
                    ...
                    <FormField
                        label={t('notifications.admin.fields.body_ar')}
                        error={error('body_ar')}
                        required
                    >
                        <Textarea
                            ...
                            onChange={(e) =>
                                form.setData('body_ar', e.target.value)
                            }
                            dir="rtl"
                            lang="ar"
                            rows={5}
                    ...
                    <FormField
                        label={t('notifications.admin.fields.body_en')}
                        error={error('body_en')}
                        required
                    >
                        <Textarea
                            ...
                            onChange={(e) =>
                                form.setData('body_en', e.target.value)
                            }
                            dir="ltr"
                            lang="en"
                            rows={5}
                    ...
                    <FormField
                        ...
                        hint={t('notifications.admin.fields.url_hint')}
                        error={error('url')}
                        optional
                    >
                        <Input
                    ...
                    <FormField
                        label={t('notifications.admin.fields.category')}
                ...
                <div className="mt-4 flex items-start gap-3 rounded-lg border p-3">
                    <Switch
                        id="is_marketing"
                        checked={form.data.is_marketing}
                        onCheckedChange={(value) =>
                            form.setData('is_marketing', value)
                        }
                        aria-describedby="is_marketing-hint"
                    />
                    <div className="space-y-1">
                        <Label htmlFor="is_marketing">
                            {t('notifications.admin.fields.is_marketing')}
                        </Label>
                        <p
                            id="is_marketing-hint"
            ...
            <SectionCard title={t('notifications.admin.fields.channels')}>
                <fieldset
                    ...
                    aria-describedby={
                        error('channels') ? 'channels-error' : undefined
                    }
                >
                    <legend className="sr-only">
                        {t('notifications.admin.fields.channels')}
                    </legend>
                    {channels.map((channel) => {
                        const id = `channel-${channel.value}`;
                        const checked =
                            channel.always ||
                            form.data.channels.includes(channel.value);
                        return (
                            <div
                                key={channel.value}
                            ...
                            >
                                <Checkbox
                                    ...
                                    checked={checked}
                                    disabled={
                                        channel.always || !channel.configured
                                    }
                                    onCheckedChange={(value) =>
                                        toggleChannel(
                                            channel.value,
                                ...
                                >
                                    ...
                                    {channel.always ? (
                                        ...
                                        >
                                            ...
                                            {t(
                                                'notifications.admin.fields.in_app_always',
                                            )}
                                        </Badge>
                                    ) : null}
                                    {!channel.configured ? (
                                        <Badge
                                            ...
                                            title={t(
                                                'notifications.admin.channel_not_configured_hint',
                                            )}
                                        >
                                            {t(
                                                'notifications.admin.channel_not_configured',
                                            )}
                                        </Badge>
                                    ) : null}
                                </Label>
                ...
                {channels.some((c) => !c.configured) ? (
                    <p className="mt-3 text-xs text-muted-foreground">
                        {t('notifications.admin.channel_not_configured_hint')}
                    </p>
                ) : null}
                {error('channels') ? (
                    <p
            ...
            <SectionCard title={t('notifications.admin.fields.audience')}>
                <div className="grid gap-4 md:grid-cols-2">
                    <FormField
                        label={t('notifications.admin.fields.audience_type')}
                        error={error('audience_type')}
                        required
                    >
                        {(controlProps) => (
                            <Select
                ...
                <div
                    className="mt-4 flex flex-wrap items-center gap-3 rounded-lg bg-muted/50 p-3"
                    aria-live="polite"
                >
                    <Users
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium">
                            {estimating ? (
                                ...
                                (estimateLabel ??
                                t(
                                    'notifications.admin.fields.estimated_recipients',
                                ))
                            )}
                        </p>
                        <p className="text-xs text-muted-foreground">
                            {t('notifications.admin.estimate_hint')}
                        </p>
                    </div>
                    <Button
                        type="button"
            ...
            >
                {previewError ? (
                    <InlineAlert tone="danger">{previewError}</InlineAlert>
                ) : null}
                {previewData ? (
                            ...
                            >
                                ...
                                >
                                    <p className="text-xs font-medium text-muted-foreground">
                                        {t(
                                            'notifications.admin.in_app_preview',
                                        )}
                                    </p>
                                    <div className="rounded-lg border bg-card p-4">
                                        <p className="text-sm font-semibold">
                                            {previewData[locale].title}
                                        </p>
                                        <p className="mt-1 text-sm whitespace-pre-line text-muted-foreground">
                                            {previewData[locale].body}
                                        </p>
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <p className="text-xs font-medium text-muted-foreground">
                                        {t('notifications.admin.email_preview')}{' '}
                                        — {t('notifications.admin.subject')}:{' '}
                                        <span
                                            dir={
                                                locale === 'ar' ? 'rtl' : 'ltr'
                                            }
                                        >
                                            {previewData[locale].email_subject}
                                        </span>
                                    </p>
                                    <EmailFrame
                                        html={previewData[locale].email_html}
                                        dir={locale === 'ar' ? 'rtl' : 'ltr'}
                                        lang={locale}
                                        title={`${t('notifications.admin.email_preview')} (${locale})`}
                                    />
                                </div>
                            </TabsContent>
                        ))}
                    ...
                    <p className="text-sm text-muted-foreground">
                        {t('notifications.admin.preview_empty')}
                    </p>
                )}
            </SectionCard>
            ...
            <FormActions>
                ...
                <Button type="submit" disabled={form.processing}>
                    ...
                    {campaign
                        ? t('notifications.admin.actions.save_changes')
                        : t('notifications.admin.actions.save_draft')}
                </Button>
            </FormActions>
        </form>
    );
