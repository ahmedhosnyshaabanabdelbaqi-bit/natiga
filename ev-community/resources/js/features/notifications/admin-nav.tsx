...
export function AdminNotificationsNav({ current }: { current: Section }) {
    ...
    const items: { key: Section; href: string; label: string; icon: LucideIcon; visible: boolean }[] = [
        { key: 'campaigns', href: index.url(), label: t('notifications.admin.campaigns'), icon: Send, visible: can(VIEW, auth) },
        { key: 'deliveries', href: deliveriesIndex.url(), label: t('notifications.admin.deliveries'), icon: LayoutList, visible: can(VIEW, auth) },
        { key: 'templates', href: templatesIndex.url(), label: t('notifications.admin.templates'), icon: Mail, visible: can(VIEW, auth) },
        { key: 'inbox', href: inbox.url(), label: t('notifications.admin.inbox'), icon: Inbox, visible: true },
    ];
    ...
    return (
        <nav aria-label={t('notifications.title')} className="-mx-1 overflow-x-auto">
                    ...
                    <li key={item.key}>
                        <Link
                            ...
                            className={cn(
                                ...
                                item.key === current ? 'border-brand text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground',
                            )}
                        >
