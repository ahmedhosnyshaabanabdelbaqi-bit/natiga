...
const emptySelection: VehicleSelection = {
    make_id: null,
    model_id: null,
    variant_id: null,
    year: null,
...
export function VehicleSelector({
    ...
    const trigger = (
        <Button
            type="button"
            variant={vehicle ? 'outline' : 'default'}
            size={variant === 'compact' ? 'sm' : 'default'}
            onClick={() => setOpen(true)}
        >
            <Car className="size-4" aria-hidden="true" />
            {vehicle
                ? t('vehicles.public.change')
                : t('vehicles.public.select_title')}
        </Button>
    );
    ...
    const summary = vehicle ? (
        ...
        <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">
                {t('vehicles.public.select_description')}
            </p>
        </div>
    );
    ...
    const actions = fromGarage ? (
        ...
        >
            <Link href="/account/garage">
                {t('vehicles.public.manage_in_garage')}
            </Link>
        </Button>
    ) : (
...
function SelectorDialog({
    ...
    const { catalog, loading, failed, retry } = useVehicleCatalog(
        initialCatalog,
        open,
    );
            ...
            <DialogContent className="sm:max-w-xl">
                ...
                {failed ? (
                    ...
                    <VehiclePicker
                        ...
                        errors={{
                            make: errors?.make_id,
                            model: errors?.model_id,
                            variant: errors?.variant_id,
                            year: errors?.year,
                        }}
                    />
                )}
                ...
                <DialogFooter>
                    ...
                    <Button
                        type="button"
                        disabled={
                            submitting || !value.make_id || !value.model_id
                        }
                        onClick={() => {
                            setSubmitting(true);
