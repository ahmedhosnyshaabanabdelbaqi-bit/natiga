...
function load(locale: string): Promise<VehicleCatalog> {
    ...
    if (!pending) {
        pending = fetch(vehicleDataRoute(locale).url, {
...
export function useVehicleCatalog(
    initial?: VehicleCatalog | null,
    enabled = true,
): UseVehicleCatalog {
    const { locale } = useLocale();
    const [catalog, setCatalog] = useState<VehicleCatalog | null>(
        initial ?? null,
    );
