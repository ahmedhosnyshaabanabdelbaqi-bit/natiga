...
type Errors = Partial<
    Record<'make' | 'model' | 'variant' | 'year', string | undefined>
>;
...
export function VehiclePicker({
    ...
    return (
        <div className={cn('grid gap-4 sm:grid-cols-2', className)}>
            <FormField
                id={`${idPrefix}-make`}
                label={t('vehicles.selector.make')}
                required
                error={errors.make}
            >
                <OptionSelect
                    ...
                    disabled={disabled}
                    options={catalog.makes.map((item) => ({
            ...
            <FormField
                id={`${idPrefix}-model`}
                label={t('vehicles.selector.model')}
                required
                error={errors.model}
            >
                <OptionSelect
                    ...
                    placeholder={
                        make
                            ? t('vehicles.selector.choose_model')
                            : t('vehicles.selector.choose_make_first')
                    }
                    disabled={disabled || !make}
                    options={(make?.models ?? []).map((item) => ({
                        value: String(item.id),
                        label: item.name,
                    }))}
                    onChange={(next) =>
            ...
            {withVariant ? (
                <FormField
                    id={`${idPrefix}-variant`}
                    label={t('vehicles.selector.variant')}
                    optional
                    error={errors.variant}
                    hint={
                        model && model.variants.length === 0
                            ? t('vehicles.selector.no_variants')
                            : undefined
                    }
                >
                    <OptionSelect
                        ...
                        options={[
                            {
                                ...
                                label: t('vehicles.selector.unknown_variant'),
                            },
                        ...
                        onChange={(next) => {
                            const chosen =
                                next === UNKNOWN_VARIANT
                                    ? undefined
                                    : findVariant(model, Number(next));
                            const keepYear =
                                value.year !== null &&
                                (!chosen ||
                                    yearOptions(catalog, chosen).includes(
                                        value.year,
                                    ));
                            onChange({
                                ...value,
                                variant_id: chosen ? chosen.id : null,
                                year: keepYear ? value.year : null,
                            });
                        }}
                    />
                </FormField>
            ...
            {withYear ? (
                <FormField
                    id={`${idPrefix}-year`}
                    label={t('vehicles.selector.year')}
                    required={yearRequired}
                    optional={!yearRequired}
                    error={errors.year}
                >
                    <OptionSelect
                        ...
                        onChange={(next) =>
                            onChange({ ...value, year: Number(next) })
                        }
                    />
                </FormField>
            ) : null}
