...
function ValueCell({ value }: { value: unknown }) {
    ...
    if (value === undefined || value === null || value === '') {
        return (
            <span className="text-xs text-muted-foreground">
                {t('audit.logs.detail.empty_value')}
            </span>
        );
    }
    const text =
    ...
    return (
        <pre
            className="max-h-40 overflow-auto font-mono text-xs break-all whitespace-pre-wrap"
            dir="ltr"
        >
            {text}
        </pre>
        ...
        >
            ...
            >
                <SheetHeader>
                    <SheetTitle>
                        {entry
                            ? t('audit.logs.detail.title', { id: entry.id })
                            : ''}
                    </SheetTitle>
                    <SheetDescription>
                        {entry ? <Code>{entry.action}</Code> : null}
                    </SheetDescription>
                </SheetHeader>
                <div className="grid gap-6 px-4 pb-6">
                    {failed ? (
                        <ErrorState
                            title={t('audit.logs.detail.error')}
                        ...
                        <>
                            <dl className="grid gap-3 sm:grid-cols-2">
                                ...
                                <Row label={t('audit.logs.detail.actor')}>
                                    {detail.actor ? (
                                        ...
                                        tOr(
                                            `audit.logs.actor_types.${detail.actor_type}`,
                                            humanize(detail.actor_type),
                                        )
                                    )}
                                </Row>
                                ...
                                <Row label={t('audit.logs.detail.reason')}>
                                    {detail.reason ?? '—'}
                                </Row>
                                <Row label={t('audit.logs.detail.ip')}>
                                    {detail.ip_address ? (
                                        <Code>{detail.ip_address}</Code>
                                    ) : (
                                        '—'
                                    )}
                                </Row>
                            ...
                            <section
                                ...
                                aria-label={t('audit.logs.detail.changes')}
                            >
                                <h3 className="text-sm font-medium">
                                    {t('audit.logs.detail.changes')}
                                </h3>
                                {keys.length === 0 ? (
                                    <p className="text-sm text-muted-foreground">
                                        {t('audit.logs.detail.no_changes')}
                                    </p>
                                ) : (
