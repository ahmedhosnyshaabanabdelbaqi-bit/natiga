...
export function NoResults({ onReset }: { onReset?: () => void }) {
    return (
        <EmptyState
            ...
            action={onReset ? <button type="button" onClick={onReset} className="text-sm font-medium text-brand underline-offset-4 hover:underline">{t('core.actions.reset')}</button> : undefined}
        />
    );
}
