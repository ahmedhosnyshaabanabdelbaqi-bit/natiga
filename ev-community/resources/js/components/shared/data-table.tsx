...
import type { AnyPaginated, PaginationMeta } from '@/components/shared/pagination';
...
export function useInertiaLoading(): boolean {
    ...
    useEffect(() => {
        ...
        const offStart = router.on('start', (event) => {
            ...
            if (visit.method === 'get' && !visit.prefetch && visit.url.pathname === path) {
                setLoading(true);
            }
        });
...
const INTERACTIVE_SELECTOR = 'a,button,input,select,textarea,label,[role="checkbox"],[role="menuitem"],[data-no-row-click]';

function PerPageSelect({ id, meta, options }: { id: string; meta: PaginationMeta; options: number[] }) {
    const { patch } = useQueryState();
    const list = options.includes(meta.per_page) ? options : [...options, meta.per_page].sort((a, b) => a - b);
    const labelId = `${id}-per-page`;
    return (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <span id={labelId}>{t('ui.table.rows_per_page')}</span>
            <Select value={String(meta.per_page)} onValueChange={(value) => patch({ per_page: value })}>
                <SelectTrigger size="sm" aria-labelledby={labelId} className="w-20 tabular">
                    <SelectValue />
                </SelectTrigger>
                <SelectContent>
                    {list.map((size) => (
...
export function DataTable<T>({
    ...
    const [hidden, setHidden] = useState<string[]>(
        () => (typeof window === 'undefined' ? null : readHidden(id)) ?? columns.filter((column) => column.defaultHidden).map((column) => column.key),
    );
    // Selection is tied to the current data set: a new page/result set clears it.
    const [selection, setSelection] = useState<{ source: unknown; keys: RowKey[] }>({ source: data, keys: [] });
    const selected = selection.source === data ? selection.keys : [];
    const setSelected = (keys: RowKey[]) => setSelection({ source: data, keys });
    ...
    const visible = columns.filter((column) => column.required || !hidden.includes(column.key));
    const mobileColumns = visible.filter((column) => !column.hideOnMobile);
    const toggleColumn = (key: string, shown: boolean) => {
        const next = shown ? hidden.filter((item) => item !== key) : [...hidden.filter((item) => item !== key), key];
        setHidden(next);
        writeHidden(id, next);
    };
    ...
    const onSort = (column: DataTableColumn<T>) => {
        ...
        query.patch({ sort: key, direction: sort === key && direction === 'asc' ? 'desc' : 'asc' });
    };

    const renderCell = (column: DataTableColumn<T>, row: T, index: number): ReactNode => (column.cell ? column.cell(row, index) : defaultCell(row, column.key));
        ...
        interactive
            ? {
                  ...
                  onAuxClick: (event: MouseEvent<HTMLElement>) => {
                      if (event.button === 1 && rowHref && !(event.target as HTMLElement).closest(INTERACTIVE_SELECTOR)) {
                          activate(row, true);
                      }
                  },
    ...
    const toggleRow = (key: RowKey, checked: boolean) => setSelected(checked ? [...selected.filter((item) => item !== key), key] : selected.filter((item) => item !== key));

    const header =
        toolbar || (columnToggle && !isMobile) || selected.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2">
                {toolbar ? <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">{toolbar}</div> : null}
                {selected.length > 0 ? (
                    <div className="flex flex-wrap items-center gap-2 rounded-lg border bg-muted/40 px-3 py-1.5 text-sm" aria-live="polite">
                        <span className="font-medium tabular">{t('ui.table.selected', { count: selected.length })}</span>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-52">
                            ...
                            {columns.map((column) => (
                                <DropdownMenuCheckboxItem
                                    key={column.key}
                                    checked={column.required || !hidden.includes(column.key)}
    ...
    const empty = emptyState ?? (filtered ? <NoResults onReset={onResetFilters ?? (() => query.reset(SORT_KEYS))} /> : <EmptyState title={emptyTitle} description={emptyDescription} action={emptyAction} />);
    ...
    if (isMobile) {
        return (
            <div className={cn('flex flex-col gap-3', className)} data-slot="data-table">
                {header}
                {loading ? (
                    <SkeletonCards count={3} className="grid-cols-1 sm:grid-cols-1 lg:grid-cols-1" />
                ) : rows.length === 0 ? (
                    empty
                ) : (
                            ...
                            return (
                                <li
                                    ...
                                    className={cn(
                                        ...
                                        interactive && 'cursor-pointer focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:outline-none',
                                        isSelected && 'border-brand/50 bg-brand-soft/20',
                                        rowClassName?.(row),
                                    )}
                                    {...rowProps(row)}
                                >
                                    <div className="flex items-start gap-3">
                                        {selectable ? (
                                            <Checkbox className="mt-1" checked={isSelected} onCheckedChange={(checked) => toggleRow(key, checked === true)} aria-label={t('ui.table.select_row')} />
                                        ) : null}
                                        <div className="min-w-0 flex-1">
                                            <div className="font-medium break-words">{mobileTitle ? mobileTitle(row) : titleColumn ? renderCell(titleColumn, row, index) : null}</div>
                                            {rest.length > 0 ? (
                                                <dl className="mt-2 grid grid-cols-[minmax(0,auto)_minmax(0,1fr)] gap-x-3 gap-y-1 text-sm">
                                                    {rest.map((column) => (
                                                        <Fragment key={column.key}>
                                                            <dt className="text-muted-foreground">{column.header}</dt>
                                                            <dd className={cn('min-w-0 text-end break-words', column.className)}>{renderCell(column, row, index)}</dd>
                                                        </Fragment>
                                                    ))}
                                                </dl>
                                            ) : null}
                ...
                {footer ? <div className="rounded-xl border bg-card">{footer}</div> : null}
            </div>
        );
    }
                    ...
                    <Table>
                        ...
                        <TableHeader>
                            <TableRow className="hover:bg-transparent">
                                {selectable ? (
                                    <TableHead className="w-10">
                                        <Checkbox
                                            ...
                                            onCheckedChange={(checked) => setSelected(checked === true ? allKeys : [])}
                                ...
                                {visible.map((column) => {
                                    const key = column.sortKey ?? column.key;
                                    const isSorted = column.sortable === true && sort === key;
                                    const SortIcon = isSorted ? (direction === 'asc' ? ArrowUp : ArrowDown) : ArrowUpDown;
                                    return (
                                        <TableHead
                                            key={column.key}
                                            aria-sort={column.sortable ? (isSorted ? (direction === 'asc' ? 'ascending' : 'descending') : 'none') : undefined}
                                        ...
                                        >
                                            {column.sortable ? (
                                                <button
                                                    ...
                                                    aria-label={t('ui.table.sort_by', { column: typeof column.header === 'string' ? column.header : column.key })}
                                                >
                                                    {column.header}
                            ...
                            {loading
                                ? Array.from({ length: skeletonRows }, (_, index) => (
                                      <TableRow key={`skeleton-${index}`} className="hover:bg-transparent">
                                          ...
                                          {visible.map((column) => (
                                              <TableCell key={column.key} className={dense ? 'py-2' : 'py-3'}>
                                ...
                                : rows.map((row, index) => {
                                      ...
                                      return (
                                          <TableRow
                                              key={key}
                                              data-state={isSelected ? 'selected' : undefined}
                                              className={cn(interactive && 'cursor-pointer outline-none focus-visible:bg-muted/60', rowClassName?.(row))}
                                              {...rowProps(row)}
                                          >
                                              {selectable ? (
                                                  <TableCell className="w-10">
                                                      <Checkbox checked={isSelected} onCheckedChange={(checked) => toggleRow(key, checked === true)} aria-label={t('ui.table.select_row')} />
                                                  </TableCell>
                                              ) : null}
                                              {visible.map((column) => (
                                                  <TableCell key={column.key} className={cn(dense ? 'py-1.5' : 'py-2.5', alignClass(column.align), column.className)}>
