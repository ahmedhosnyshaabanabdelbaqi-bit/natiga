...
export function ProgressBar({ value, max = 100, label, format = 'percent', hint, tone = 'brand', size = 'md', className }: Props) {
    ...
    const text =
        format === 'none'
            ? null
            : format === 'fraction'
              ? t('ui.progress.of', { value: formatNumber(value, 0), max: formatNumber(max, 0) })
        ...
        <div className={cn('flex flex-col gap-1.5', className)}>
            {label || text ? (
                <div className="flex items-center justify-between gap-3 text-sm">
                    ...
                    {text ? <span className="shrink-0 tabular text-muted-foreground">{text}</span> : null}
                </div>
            ) : null}
            <Progress value={value} max={safeMax} aria-label={ariaLabel} className={heights[size]} indicatorClassName={toneSolid[tone]} />
