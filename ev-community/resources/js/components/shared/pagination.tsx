...
import type { Paginated, ResourcePaginated, SimplePaginated } from '@/types/pagination';
...
export type AnyPaginated<T> = Paginated<T> | SimplePaginated<T> | ResourcePaginated<T>;

function isResourcePaginated<T>(data: AnyPaginated<T> | T[]): data is ResourcePaginated<T> {
    return !Array.isArray(data) && 'meta' in data && typeof data.meta === 'object' && data.meta !== null && 'current_page' in data.meta;
}

function isLengthAware<T>(data: AnyPaginated<T> | T[]): data is Paginated<T> {
    return !Array.isArray(data) && 'last_page' in data && 'current_page' in data;
}

function isSimple<T>(data: AnyPaginated<T> | T[]): data is SimplePaginated<T> {
    return !Array.isArray(data) && 'current_page' in data && !('last_page' in data);
}

/** Accepts every Laravel paginator JSON shape (or a plain array) and returns rows + meta. */
export function normalizePaginated<T>(data: AnyPaginated<T> | T[]): { rows: T[]; meta: PaginationMeta | null } {
...
function pageRange(current: number, last: number, siblings: number): Array<number | 'gap'> {
...
function isMeta<T>(data: AnyPaginated<T> | PaginationMeta): data is PaginationMeta {
    return 'has_next' in data && 'has_prev' in data;
}

/** Laravel paginator navigation that preserves the current filters, state and scroll. */
export function Pagination<T>({ data, showSummary = true, siblings = 1, only, className }: Props<T>) {
    const { href } = useQueryState();
    const meta = isMeta(data) ? data : normalizePaginated(data).meta;
    if (!meta || (!meta.has_prev && !meta.has_next && (meta.last_page ?? 1) <= 1)) {
        return null;
    }
    const linkProps = { preserveState: true, preserveScroll: true, only } as const;
    const pageHref = (page: number) => href({ page: page === 1 ? null : String(page) });

    return (
        <div className={cn('flex flex-col items-center justify-between gap-3 sm:flex-row', className)}>
            {showSummary && meta.total !== null && meta.from !== null && meta.to !== null ? (
                <p className="text-sm text-muted-foreground tabular">
                    {t('core.labels.showing', { from: formatNumber(meta.from, 0), to: formatNumber(meta.to, 0), total: formatNumber(meta.total, 0) })}
                </p>
            ) : (
                <span />
            )}
            <PaginationRoot className="mx-0 w-auto">
                <PaginationContent>
                    <PaginationItem>
                        {meta.has_prev ? (
                            <PaginationPrevious asChild>
                                <Link href={pageHref(meta.current_page - 1)} {...linkProps} />
                            </PaginationPrevious>
                        ) : (
                    ...
                    {meta.last_page !== null
                        ? pageRange(meta.current_page, meta.last_page, siblings).map((page, index) =>
                              page === 'gap' ? (
                                  ...
                                  <PaginationItem key={page}>
                                      <PaginationLink asChild isActive={page === meta.current_page} aria-label={t('ui.pagination.page', { page })}>
                                          <Link href={pageHref(page)} {...linkProps}>
                                              {formatNumber(page, 0)}
                                          </Link>
                                      </PaginationLink>
                                  </PaginationItem>
