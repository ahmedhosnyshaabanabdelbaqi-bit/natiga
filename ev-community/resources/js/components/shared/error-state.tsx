...
export function ErrorState({
    ...
    kind?: 'error' | 'permission' | 'connection';
    title?: string;
    ...
    const defaultTitle =
        kind === 'permission'
            ? t('core.states.permission_denied')
            : kind === 'connection'
              ? t('core.states.connection_error')
              : t('core.states.error');
    return (
        <div
            role="alert"
            className="flex flex-col items-center justify-center rounded-xl border border-danger/30 bg-danger-soft/40 px-6 py-10 text-center"
        >
