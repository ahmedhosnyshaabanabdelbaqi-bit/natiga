...
export function SectionCard({ title, description, actions, footer, children, flush = false, className, contentClassName }: SectionCardProps) {
    ...
    return (
        <Card className={cn('gap-0 py-0 shadow-card', className)}>
            ...
            {children !== undefined && children !== null ? (
                <CardContent className={cn(flush ? 'p-0' : 'px-4 py-4 md:px-6', contentClassName)}>{children}</CardContent>
            ) : null}
            {footer ? <CardFooter className="border-t px-4 py-3 md:px-6">{footer}</CardFooter> : null}
        </Card>
    );
}
...
function renderValue(item: DescriptionItem): ReactNode {
    ...
    if (type === 'code' && (typeof value === 'string' || typeof value === 'number')) {
        return <Code>{value}</Code>;
    }
    if (type === 'money' && (typeof value === 'string' || typeof value === 'number')) {
        return <Money amount={value} currency={item.currency} />;
    }
    if ((type === 'date' || type === 'datetime' || type === 'relative') && typeof value === 'string') {
...
export function DescriptionList({ items, columns = 2, layout = 'stacked', className }: DescriptionListProps) {
    const visible = items.filter((item) => !item.hidden);
    return (
        <dl className={cn('grid gap-x-6 gap-y-4', columns === 2 && 'sm:grid-cols-2', layout === 'inline' && 'gap-y-2', className)}>
            {visible.map((item, index) => (
                <div
                    ...
                    className={cn(
                        ...
                        layout === 'inline' && 'flex items-baseline justify-between gap-4 border-b border-dashed py-1.5 last:border-0',
                    )}
                >
                    <dt className={cn('text-xs font-medium text-muted-foreground', layout === 'inline' && 'shrink-0 text-sm')}>{item.label}</dt>
                    <dd className={cn('mt-0.5 text-sm break-words', layout === 'inline' && 'mt-0 text-end')}>{renderValue(item)}</dd>
                </div>
            ))}
        </dl>
    );
