...
export default function IncidentsIndex({
    ...
    const filters: FilterDefinition[] = [
        {
            key: 'q',
            type: 'search',
            label: t('operations.incidents.filters.q'),
            className: 'md:w-64',
        },
        {
            key: 'severity',
            type: 'select',
            label: t('operations.incidents.filters.severity'),
        ...
        },
        {
            ...
            options: [
                { value: 'me', label: t('operations.incidents.filters.mine') },
                ...owners.map((person) => ({
                    value: person.id,
                    label: person.name,
                })),
    ...
    const columns: DataTableColumn<IncidentSummary>[] = [
        ...
        {
            key: 'title',
            header: t('operations.incidents.fields.title'),
            cell: (incident) => (
        ...
        {
            key: 'severity',
            header: t('operations.incidents.fields.severity'),
            cell: (incident) => <SeverityBadge severity={incident.severity} />,
        },
        {
            key: 'status',
            header: t('operations.incidents.fields.status'),
            cell: (incident) => (
        ...
        {
            key: 'owner',
            header: t('operations.incidents.fields.owner'),
            cell: (incident) =>
                incident.owner?.name ?? (
        ...
        {
            key: 'affected_module',
            header: t('operations.incidents.fields.affected_module'),
            hideOnMobile: true,
            cell: (incident) =>
        ...
        {
            key: 'started_at',
            header: t('operations.incidents.fields.started_at'),
            cell: (incident) => (
                <DateTime
        ...
        {
            key: 'resolved_at',
            header: t('operations.incidents.fields.resolved_at'),
            defaultHidden: true,
            cell: (incident) => (
                <DateTime
        ...
        <>
            ...
            <div className="grid gap-6">
                            ...
                            {can(['operations.view', 'operations.manage']) ? (
                                <Button variant="outline" asChild>
                                    <Link href={operationsIndex().url}>
                                        ...
                                        />
                                        {t(
                                            'operations.incidents.actions.exceptions',
                                        )}
                                    </Link>
                                </Button>
                            ) : null}
                            {abilities.manage ? (
                                <Button asChild>
                                    <Link href={create().url}>
                                        ...
                                        />
                                        {t(
                                            'operations.incidents.actions.create',
                                        )}
                                    </Link>
                                </Button>
                            ) : null}
                ...
                <DataTable
                        ...
                        <FiltersBar filters={filters}>
                            ...
                            >
                                <SelectTrigger
                                    ...
                                    aria-label={t(
                                        'operations.incidents.filters.status',
                                    )}
                                >
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="active">
                                        {t(
                                            'operations.incidents.filters.active',
                                        )}
                                    </SelectItem>
                                    ...
                                    {statuses.map((status) => (
                                        <SelectItem key={status} value={status}>
                                            {t(
                                                `operations.incident_status.${status}`,
                                            )}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                    ...
                    emptyDescription={t(
                        'operations.incidents.empty.description',
                    )}
                    emptyAction={
                        abilities.manage ? (
