...
/**
 * Read-only staff view of member vehicles. VINs are never listed; a VIN can only be *looked up*
 * (POST, hashed, audited) and the matching vehicle is shown.
 */
class MemberVehicleController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $filters = [
            ...
            'status' => in_array($request->query('status'), VehicleStatus::values(), true) ? $request->query('status') : null,
        ];
        $vinHash = $request->session()->get(self::VIN_LOOKUP_SESSION_KEY);

        $query = MemberVehicle::query()->with(['make', 'model', 'variant', 'user', 'membership']);
        if (is_string($vinHash) && strlen($vinHash) === 64) {
        ...
        return Inertia::render('admin/vehicles/members/index', [
            'vehicles' => $paginator,
            'filters' => $filters,
            'vinLookup' => is_string($vinHash) ? ['active' => true, 'matched' => $paginator->total() > 0] : null,
    ...
    public function vinLookup(VinLookupRequest $request): RedirectResponse
    {
        ...
        $this->audit->log('vehicles.vin_lookup', null, new: ['vin_hash_prefix' => substr($hash, 0, 12)], actor: $request->user());
    ...
    public function show(Request $request, MemberVehicle $vehicle): Response
    {
        ...
        $vehicle->load(['make', 'model', 'variant.battery', 'variant.acConnector', 'variant.dcConnector', 'battery', 'user', 'membership']);
        ...
        return Inertia::render('admin/vehicles/members/show', [
            ...
            'memberUrl' => $vehicle->membership ? '/admin/members/'.$vehicle->membership->public_id : null,
        ]);
    }
}
