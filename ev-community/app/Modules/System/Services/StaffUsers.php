...
final class StaffUsers
{
    ...
    public const ALLOWED_FILTERS = ['q', 'role', 'status', 'mfa', 'sort', 'dir'];
    ...
    public function scope(): Builder
    {
        return User::query()->where(function (Builder $q) {
            $q->whereHas('roles', fn (Builder $r) => $r->where('name', '!=', 'member'))
                ->orWhereHas('permissions', fn (Builder $p) => $p->whereIn('name', ['admin.access', 'partner.access']));
        });
    }
    ...
    /** Roles an admin can pick in the UI (member is excluded; super roles flagged). */
    public function assignableRoles(): array
    {
        $meta = RoleMeta::query()->get()->keyBy('role_id');

        return Role::query()->where('guard_name', 'web')->where('name', '!=', 'member')->orderBy('id')->get()->map(function (Role $role) use ($meta) {
    ...
    /** Permission definitions grouped by module for the overrides picker. */
    public function permissionGroups(): array
    {
        $out = [];
        foreach (PermissionRegistry::grouped() as $module => $permissions) {
            $out[] = ['module' => $module, 'permissions' => collect($permissions)->map(fn ($d, $key) => ['key' => $key, 'label' => $d['label']])->values()->all()];
        }
