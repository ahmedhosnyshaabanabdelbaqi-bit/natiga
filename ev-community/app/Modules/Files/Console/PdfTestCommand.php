...
class PdfTestCommand extends Command
{
    ...
    public static function sampleData(string $locale, ?string $qrDataUri = null, ?array $productNames = null): array
    {
        ...
        foreach (array_values($names) as $i => $name) {
            $quantity = $i + 1;
            $unitPrice = $prices[$i % count($prices)];
            $lineTotal = Money::mul($unitPrice, $quantity, $currency);
            $lineTotals[] = $lineTotal;
            $items[] = [
                'name' => $name,
                'sku' => $skus[$i % count($skus)].($i >= count($skus) ? '-'.$i : ''),
                'quantity' => $quantity,
                'unit_price_formatted' => Money::format($unitPrice, $currency, $locale),
                'line_total_formatted' => Money::format($lineTotal, $currency, $locale),
            ];
        }
        ...
        return [
            ...
            'items' => $items,
            'grand_total_formatted' => Money::format(Money::toDecimal(Money::sum($lineTotals, $currency)), $currency, $locale),
