...
Tests:        83 passed (336 assertions)
Duration:     47.5s
```
...
| **Feature** | 13 | 62 | سيناريوهات كاملة على PostgreSQL حقيقي |
...
| `Feature/PosEndpointsTest` | 5 | مسارات الشاشة كما يستدعيها المتصفح فعلًا: معرّفات نصية في الـquery، بحث بلا مصطلح، باركود مجهول |
...
npm run build        # typecheck + بناء  → نظيف
npm run smoke        # دورة بيع كاملة في متصفح حقيقي
npm run sweep        # كل الشاشات، نهاري وليلي: أخطاء، شاشة فارغة، تمرير أفقي
```

### أخطاء كشفها الفحص البصري

| الخطأ | كيف ظهر | الإصلاح |
|---|---|---|
| `500` على `/pos/search` عند تمرير `warehouse_id` | معرّفات الـquery نصوص، والتوقيع الصارم يرفضها | تحويل المعرّفات عند حدود الطلب + `PosEndpointsTest` |
| `unit_price` بصيغتين مختلفتين بين `/scan` و`/search` | `"95.00"` مقابل `"95.0000"` | توحيد التقريب لمقياس العملة |
| كميات بست خانات عشرية بعد دمج مسحتين | الخادم يرفضها بحق | `normalizeQty` عند كل كتابة كمية |
| رأس الشاشة لا يتحدث بعد فتح وردية بـ Esc | الإغلاق لم يُعد قراءة الحالة | توحيد مسار إغلاق النوافذ |
| أزرار أيقونية بلا اسم مقروء | تعذّر استهدافها في الفحص | `aria-label` على كل زر أيقوني |
| الخط من CDN خارجي | فشل تحميله يعني خطًا بديلًا | استضافة Cairo محليًا (92 كيلوبايت) |
