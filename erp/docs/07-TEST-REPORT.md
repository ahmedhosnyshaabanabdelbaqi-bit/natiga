...
## ٩ب. فحص اللغة والاتجاه

**الأداة:** Playwright + Chromium · قياس بالبكسل لموضع القائمة الجانبية في اللغتين.

**النتيجة: 22 من 22 فحصًا · صفر أخطاء متصفح.**

| الفحص | النتيجة |
|---|---|
| العربية: `dir=rtl` | ✅ |
| العربية: القائمة على **اليمين** | ✅ `left=1186 right=1440` في نافذة 1440 |
| الإنجليزية: `dir=ltr` | ✅ |
| الإنجليزية: القائمة على **الشمال** | ✅ `left=0 right=254` |
| عدم تداخل القائمة مع المحتوى في اللغتين | ✅ |
| ترجمة نصوص الواجهة فعليًا | ✅ ظهور `Dashboard` واختفاء `لوحة التحكم` |
| سبع شاشات بالإنجليزية (عملاء، أصناف، فواتير، إقفال، تقريران، إعدادات) | ✅ كلها بالقائمة على الشمال |
| بقاء الاختيار بعد إعادة تحميل الصفحة | ✅ |
| الموبايل: القائمة المنزلقة تفتح من اليمين بالعربي ومن الشمال بالإنجليزي | ✅ |
| الموبايل: لا تمرير أفقي في اللغتين | ✅ |

### عيوب اكتُشفت بفحص اللغة وأُصلحت

1. **القائمة الجانبية كانت على الشمال في العربي** — استُخدم `inset-inline-end` بدل `inset-inline-start`، فكانت تلتصق بالجهة المعاكسة للغة. هذا هو العيب الذي لوحظ في اللقطات الأولى.
2. **الشريط العلوي على الموبايل عرضه 136 بكسل بدل 390** — بعد نقل هامش المحتوى إلى `inline-start` ظل تجاوز الموبايل يصفّر الجهة القديمة، فبقي هامش 254 بكسل محجوزًا لقائمة خارج الشاشة أصلًا، وخرج زر اللغة من حدود النافذة.
3. **حلقة إعادة رسم لا نهائية** (`Maximum update depth exceeded`) — دالة الترجمة كانت تُنشأ في كل رسم فتُعيد تشغيل `useEffect` المعتمِد عليها، مع تعيين مصفوفة نتائج جديدة في كل مرة. عُولج بتثبيت مرجع الدالة عبر `useCallback` وبعدم تعيين مصفوفة جديدة وهي فارغة أصلًا.
4. **ازدحام الشريط العلوي على الشاشات الضيقة** — أُخفي مسار التنقل (يتكرر مع عنوان الصفحة) واسم المستخدم ودوره، وصار حقل البحث مرنًا.
...
## ١٣. دورة البيع: أمر ← إذن تسليم ← فاتورة

| الاختبار | ما يثبته |
|---|---|
| `test_approving_an_order_reserves_stock_without_moving_it` | بعد الاعتماد: الرصيد 100 كما هو، المحجوز 30، المتاح 70، **ولا قيد محاسبي** |
| `test_full_cycle_order_to_delivery_to_invoice_moves_stock_once` | الإرسال يُخرج 30 (الرصيد 70)، والفاتورة المرتبطة بالإذن **لا تُخرجها ثانيةً** (الرصيد يبقى 70) |
| `test_rejected_quantity_returns_to_stock_at_its_original_cost` | 10 مرفوضة تعود للمخزن، والمتوسط يبقى 50 — عادت بتكلفتها لا بسعر بيعها |
| `test_failed_delivery_returns_everything_and_reverses_the_entry` | الرصيد يعود 100، القيد الأصلي **موسوم معكوسًا لا محذوفًا**، وقيد العكس يشير إليه، والحجز يُعاد |
| `test_delivery_note_cannot_exceed_the_remaining_order_quantity` | إذن ثانٍ بـ15 على أمر تبقّى فيه 10 → مرفوض، والكميات في الطريق محسوبة |
| `test_cancelling_an_order_releases_reservations` | المحجوز يعود 0 والمتاح 100 |
| `test_an_order_with_a_delivered_line_cannot_be_cancelled` | رفض إلغاء أمر نُفِّذ جزء منه |
| `test_approving_an_order_beyond_available_stock_is_rejected_when_backorder_is_off` | 150 على رصيد 100 بلا تأجيل → مرفوض |
| `test_the_api_drives_the_whole_cycle_and_enforces_permissions` | الدورة كاملة عبر الـAPI، والتعرض الائتماني يعكس الأمر المعتمد غير المفوتر (960)، ومستخدم بلا صلاحيات يُرفض بـ403 |
| `test_cost_is_not_sent_in_the_delivery_note_api_to_unauthorised_users` | `unit_cost` **غائب من الاستجابة** لمن لا يملك صلاحية التكلفة |

### المسار المحاسبي — مُتحقَّق منه بالأرقام

```
استلام 100 قطعة × 50            المخزون 5000
أمر بيع 30 معتمد                لا قيد — حجز فقط (المتاح 70)
إذن تسليم 30 (إرسال)            مسلّمة غير مفوترة 1500 / المخزون 3500
رفض 10 عند التسليم              المخزون 4000 / مسلّمة غير مفوترة 1000
فاتورة على الإذن                تكلفة المبيعات 1500 / مسلّمة غير مفوترة 0
```

البضاعة التي خرجت ولم تُفوتر **لا تبقى في المخزون ولا تصير تكلفة** — تظهر في حساب
أصول مستقل (`1110`) حتى تُفوتر. بلا هذا الحساب إما يُخصم المخزون مرتين أو لا تُرحَّل التكلفة أبدًا.

### تشغيل فعلي في المتصفح

الدورة نُفِّذت يدويًا على بيانات تجريبية عبر Nginx/Vite وتم التحقق من الدفاتر بعدها:

| بعد الخطوة | النتيجة |
|---|---|
| أمر بيع 5 قطع معتمد من الشاشة | `SO-2026-000001` حالته «معتمد»، والتسليم والفوترة «لم يبدأ» |
| إذن تسليم وإرسال | تكلفة السطر ثُبّتت عند 50.00 |
| تأكيد بتسليم 3 ورفض 2 | حالة «مُسلَّم جزئيًا»، وسبب الرفض محفوظ |
| — التحقق — | مسلّمة غير مفوترة = 150.00 (3×50) · حركات 6749 = أرصدة 6749 · قيمة المخزون 293,720.00 = الدفتر 293,720.00 |
| فاتورة من الإذن | `is_stock_owner = false`، التكلفة 150.00، الإذن وُسم مفوترًا |
| — التحقق — | مسلّمة غير مفوترة = **0.00** · المخزون 293,720.00 = الفعلي · قيود غير متوازنة = **0** |

---

## ١٤. تجربة النشر الفعلية
