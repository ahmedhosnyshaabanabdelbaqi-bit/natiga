...
class CreditService
{
    ...
    public function aging(int $companyId, ?int $customerId = null, ?string $asOf = null): array
    {
        ...
        $rows = DB::table('sales_invoices')
            ...
            ->selectRaw("COALESCE(SUM(CASE WHEN due_date IS NULL OR due_date >= ? THEN total_amount - paid_amount - returned_amount ELSE 0 END), 0) AS bucket_current", [$asOf])
            ->selectRaw("COALESCE(SUM(CASE WHEN due_date < ? AND due_date >= ? - INTERVAL '30 days' THEN total_amount - paid_amount - returned_amount ELSE 0 END), 0) AS bucket_1_30", [$asOf, $asOf])
            ->selectRaw("COALESCE(SUM(CASE WHEN due_date < ? - INTERVAL '30 days' AND due_date >= ? - INTERVAL '60 days' THEN total_amount - paid_amount - returned_amount ELSE 0 END), 0) AS bucket_31_60", [$asOf, $asOf])
            ->selectRaw("COALESCE(SUM(CASE WHEN due_date < ? - INTERVAL '60 days' AND due_date >= ? - INTERVAL '90 days' THEN total_amount - paid_amount - returned_amount ELSE 0 END), 0) AS bucket_61_90", [$asOf, $asOf])
            ->selectRaw("COALESCE(SUM(CASE WHEN due_date < ? - INTERVAL '90 days' THEN total_amount - paid_amount - returned_amount ELSE 0 END), 0) AS bucket_90_plus", [$asOf])
            ->groupBy('customer_id')
