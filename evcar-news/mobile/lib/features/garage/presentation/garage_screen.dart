import 'package:flutter/material.dart';

import '../../../app/router/app_routes.dart';
import '../../../core/l10n/l10n.dart';
import '../../../shared/widgets/under_construction_view.dart';
import '../../auth/presentation/auth_gate.dart';

/// My garage (`/garage`): the user's cars with variant and market. Requires an account (guests see why and can sign in).
///
/// NOT IMPLEMENTED YET — honest placeholder owned by the `garage` feature.
/// Replace this file (keep the class name and constructor, or update
/// lib/app/router/app_router.dart in the same change).
class GarageScreen extends StatelessWidget {
  const GarageScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(context.l10n.garageTitle)),
      body: AuthGate(
        returnTo: AppRoutes.garage,
        builder: (context, user) => const UnderConstructionView(requestedPath: AppRoutes.garage),
      ),
    );
  }
}
